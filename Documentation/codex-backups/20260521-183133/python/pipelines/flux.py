...
def _dequant_scaled_fp8_weight(
    ...
    *,
    ...
    # Comfy old scaled-FP8 checkpoints store FP8 codes in a wider tensor dtype.
    # Recreate Comfy's cast-to-FP8 + scale path, then hand Diffusers full weights.
    if weight.dtype != storage_dtype:
        weight = weight.to(storage_dtype)
    out = weight.to(target_dtype) * scale.to(target_dtype)
