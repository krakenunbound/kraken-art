...
def _ensure_pipeline(diffusion_model: str, vae_name: str | None, te1: str | None, te2: str | None):
    ...
    log.info("FLUX pipeline load starting (first load ~60s: transformer + T5 FP8 dequant)")
    ...
    # ComfyUI/A1111-style safetensors use keys like
    # `model.diffusion_model.double_blocks.0.img_attn.norm.key_norm.scale`,
    # but the diffusers `FluxTransformer2DModel` expects names like
    # `time_text_embed.timestep_embedder.linear_1.weight`. Diffusers ships
    # a converter for exactly this — we run it on the raw state_dict before
    # loading, and use `assign=True` to avoid a second copy in memory.
    ...
    stripped_tx_sd = {k.removeprefix("model.diffusion_model."): v for k, v in raw_tx_sd.items()}
    del raw_tx_sd
    gc.collect()
    log.info("  converting %d transformer keys to diffusers format…", len(stripped_tx_sd))
    tx_sd = convert_flux_transformer_checkpoint_to_diffusers(stripped_tx_sd)
    del stripped_tx_sd
    gc.collect()
    ...
    # model_cpu_offload corrupts FLUX denoise on Windows (checkerboard output). Sequential
    # is slower but produces correct images until we implement ComfyUI-style staging.
    if strategy == "model_cpu_offload":
        log.warning("FLUX: model_cpu_offload produces corrupt latents on Windows — using sequential_cpu_offload")
