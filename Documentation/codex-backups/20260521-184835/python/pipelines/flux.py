...
def _scaled_linear_forward(module, x):
    ...
    scale = module._kraken_weight_scale.to(device=weight.device, dtype=weight.dtype)
    weight = weight * scale
    return torch.nn.functional.linear(x, weight, module.bias)


def _ensure_transformer_dtype_preserving_fp8(transformer, dtype: torch.dtype) -> None:
    changed = 0
    for tensor in list(transformer.parameters()) + list(transformer.buffers()):
        ...
        changed += 1
    if changed:
        log.info("transformer: coerced %d non-FP8 tensors to %s", changed, dtype)
...
def _apply_flux_weight_scales(transformer, weight_scales: dict[str, torch.Tensor]) -> None:
    ...
    for weight_key, scale in weight_scales.items():
        ...
        module.register_buffer("_kraken_weight_scale", scale.to(torch.float32), persistent=False)
...
def _ensure_pipeline(diffusion_model: str, vae_name: str | None, te1: str | None, te2: str | None):
    ...
    _ensure_transformer_dtype_preserving_fp8(transformer, dtype)
