...
# ---------------------------------------------------------------------------
# This is a verbatim vendoring of
#   diffusers/src/diffusers/models/transformers/transformer_krea2.py
# from the diffusers `main` branch (the model reports _diffusers_version
# 0.39.0.dev0). The Krea2Transformer2DModel / Krea2Pipeline classes are not in
# the released diffusers 0.38.0 that Kraken Art pins, and upgrading the whole
# library to an unreleased git-main would risk the FLUX / Wan / Z-Image / SDXL
# pipelines that depend on diffusers internals. So — exactly as Kraken already
# does for custom FLUX code (kraken_flux_attn.py, kraken_rope.py) — we vendor
# only the Krea2 model code and keep the library pinned.
#
# Only two changes vs. upstream:
#   1. relative imports (`from ...x`, `from ..x`) -> absolute `from diffusers.x`
#   2. `maybe_adjust_dtype_for_device` is shimmed (added to diffusers after
#      0.38.0; the upstream import is kept behind a try/except fallback).
# Everything else is the upstream source, so swapping back to the library
# class once diffusers 0.39 is released is a one-line import change.
# ---------------------------------------------------------------------------
...
try:
    # Present in diffusers >= 0.39 (git main). Kraken pins 0.38.0, so fall back.
    from diffusers.utils.torch_utils import maybe_adjust_dtype_for_device
except ImportError:  # pragma: no cover - exercised on diffusers 0.38.0
    _DTYPE_DOWNCAST = {torch.float64: torch.float32, torch.int64: torch.int32}
    _DTYPE_UNSUPPORTED_DEVICES = {
        torch.float64: frozenset({"mps", "npu", "neuron"}),
        torch.int64: frozenset({"mps", "npu", "neuron"}),
    }
...
class Krea2AttnProcessor:
    ...
    def __call__(
        ...
        # Kraken deviation from upstream: upstream calls dispatch_attention_fn with
        # enable_gqa=True. On torch 2.6 that flag forces SDPA onto the math kernel,
        # which materializes the full N×N attention matrix — O(n²) memory (18.6 GB
        # for one call at 1024×1536) and ~18× slower. We instead expand the KV
        # heads to match the query heads (GQA→MHA) and call SDPA directly, which
        # uses the flash / mem-efficient kernels (O(n) memory). Output is identical
        # to within bf16 noise. See dispatch_attention_fn import kept for parity.
        rep = attn.num_heads // attn.num_kv_heads
        q = query.transpose(1, 2)  # [B, S, H, D] -> [B, H, S, D]
        if rep > 1:
